        ...
        validDirections.forEach((direction) => {
          ...
          if (canDraw && math.random() >= 0.45) {
            ...
            if (
              ...
              console.debug("Skipped: " + `${currentRow}r${branchId}`);
              failedOn = `${currentRow}r${branchId}`;
              failed = true;
              return;
            }
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      <div className="flex flex-row gap-4 pt-4">
        <button hidden id="newGraph" onClick={() => newGraph()}>
          New
        </button>
        <button hidden id="newBranch" onClick={() => generateNextBranch()}>
          Add
        </button>
        <button hidden id="removeBranch" onClick={() => deleteLastBranch()}>
          Remove
        </button>
      </div>
    </div>
